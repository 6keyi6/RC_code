...
static uint8_t mpu_buff[15]; /* buffer to save imu raw data */
static uint8_t ist_buff[6];  /* buffer to save IST8310 raw data */
uint32_t IMU_Offset[IMUOffsetNum];
uint32_t IMUwriteFlashData[IMUOffsetNum] = {0};
...
/**
  ...
  */
static HAL_StatusTypeDef mpu_read_bytes(uint8_t const regAddr, uint8_t *pData, uint8_t len)
{
...
static void mpu_offset_call(void)
{
    ...
    for (i = 0; i < 400; i++)
    {
        ...
        mpu_data.Offset.ax_offset += mpu_buff[0] << 8 | mpu_buff[1];
        mpu_data.Offset.ay_offset += mpu_buff[2] << 8 | mpu_buff[3];
        mpu_data.Offset.az_offset += mpu_buff[4] << 8 | mpu_buff[5];

        mpu_data.Offset.gx_offset += mpu_buff[8] << 8 | mpu_buff[9];
        mpu_data.Offset.gy_offset += mpu_buff[10] << 8 | mpu_buff[11];
        mpu_data.Offset.gz_offset += mpu_buff[12] << 8 | mpu_buff[13];

        MPU_DELAY(5);
    }
    mpu_data.Offset.ax_offset = mpu_data.Offset.ax_offset / 400;
    mpu_data.Offset.ay_offset = mpu_data.Offset.ay_offset / 400;
    mpu_data.Offset.az_offset = mpu_data.Offset.az_offset / 400;
    mpu_data.Offset.gx_offset = mpu_data.Offset.gx_offset / 400;
    mpu_data.Offset.gy_offset = mpu_data.Offset.gy_offset / 400;
    mpu_data.Offset.gz_offset = mpu_data.Offset.gz_offset / 400;

    IMUwriteFlashData[0] = mpu_data.Offset.ax_offset;
    IMUwriteFlashData[1] = mpu_data.Offset.ay_offset;
    IMUwriteFlashData[2] = mpu_data.Offset.az_offset;
    IMUwriteFlashData[3] = mpu_data.Offset.gx_offset;
    IMUwriteFlashData[4] = mpu_data.Offset.gy_offset;
    IMUwriteFlashData[5] = mpu_data.Offset.gz_offset;
}
...
/**
  ...
  * @usage  call in main() function
	*/
static void mpu_offset_fetch(void)
{
    mpu_data.Offset.ax_offset = (uint16_t)IMU_Offset[0];
    mpu_data.Offset.ay_offset = (uint16_t)IMU_Offset[1];
    mpu_data.Offset.az_offset = (uint16_t)IMU_Offset[2];
    mpu_data.Offset.gx_offset = (uint16_t)IMU_Offset[3];
    mpu_data.Offset.gy_offset = (uint16_t)IMU_Offset[4];
    mpu_data.Offset.gz_offset = (uint16_t)IMU_Offset[5];
}
...
*@brief初始化imu mpu6500和磁性计ist3810

*@参数

*@返回

*@usage call in main（）函数
...
static uint8_t mpu_device_init(void)
{
    ...
    while (imu_Export.Temp_ReachFlag == 0)
    {
			imu_Export.Temp_ReachFlag = 1;
    };
    mpu_offset_call();
#elif Calibrate == Calibrate_Off
    mpu_offset_fetch();
#endif
...
void HAL_mpu_get_data(void)
{
    imu_Export.InfoUpdateFlag = mpu_read_bytes(MPU6500_ACCEL_XOUT_H, mpu_buff, 14);
    ...
    mpu_data.Original_Gyro.gx = ((mpu_buff[8] << 8 | mpu_buff[9]) - mpu_data.Offset.gx_offset);
    mpu_data.Original_Gyro.gy = ((mpu_buff[10] << 8 | mpu_buff[11]) - mpu_data.Offset.gy_offset);
    mpu_data.Original_Gyro.gz = ((mpu_buff[12] << 8 | mpu_buff[13]) - mpu_data.Offset.gz_offset);
    ...
    /* 2000dps -> rad/s */
    imu.Real_Gyro.wx = mpu_data.Original_Gyro.gx / 16.384f / 57.3f;
    imu.Real_Gyro.wy = mpu_data.Original_Gyro.gy / 16.384f / 57.3f;
    imu.Real_Gyro.wz = mpu_data.Original_Gyro.gz / 16.384f / 57.3f;

    imu.Gyro.gx = mpu_data.Original_Gyro.gx;
    imu.Gyro.gy = mpu_data.Original_Gyro.gy;
    imu.Gyro.gz = mpu_data.Original_Gyro.gz;
}
 ...
 */
int error = 0;
float gz_LpfAttFactor = 1;
static void MPU6500_DataCopy(imu_t *imu, imu_Export_t *imu_Export)
{
    imu_Export->temp = imu->temp;
    for (int i = 0; i < 3; i++)
    {
        imu_Export->Angle[i] = imu->Axis.Real_Angle[i];
			 

        if (imu->Axis.Real_Angle[i] - imu->Last_Axis.Last_Angle[i] < -180)
        { //经过跳变边沿。
            imu->turnCount[i]++;
        }
        if (imu->Last_Axis.Last_Angle[i] - imu->Axis.Real_Angle[i] < -180)
        {
            imu->turnCount[i]--;
        }
        imu_Export->total[i] = imu->Axis.Real_Angle[i] + (360 * imu->turnCount[i]);
       
        

			  imu->Last_Axis.Last_Angle[i] = imu->Axis.Real_Angle[i];	
				
        Filter_IIRLPF(&imu->Gyro.Original_gX[i], &imu_Export->SpeedLPF[i], gz_LpfAttFactor);
    }
}
...
{
    /* yaw    -pi----pi */
    imu.Axis.yaw = -atan2(2 * q1 * q2 + 2 * q0 * q3, -2 * q2 * q2 - 2 * q3 * q3 + 1) * 57.3 + 180.0f;
    /* pitch  -pi/2----pi/2 */
    imu.Axis.pit = -asin(-2 * q1 * q3 + 2 * q0 * q2) * 57.3 + 180.0f;
    /* roll   -pi----pi  */
    imu.Axis.rol = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2 * q2 + 1) * 57.3 + 180.0f;
    yaw =     imu.Axis.yaw;
  	pit =     imu.Axis.pit;
  	rol = imu.Axis.rol ;
    MPU6500_DataCopy(&imu, &imu_Export);
}
...
void IMU_Reset(imu_Export_t *imu_Export)
{
    ...
    {
        imu.Last_Axis.Last_Angle[i] = imu.Axis.Real_Angle[i];
        imu_Export->total[i] = imu.Axis.Real_Angle[i];
        imu.turnCount[i] = 0;
    }
}
