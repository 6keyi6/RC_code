...
void LEDYellow_MeanUpdate(void)
{
    ...
    if (Robot_Receive.Write_Msg[1] == 0)// 表示摩擦轮处于在线状态
    {
        switch (ext_game_robot_state.data.shooter_id1_17mm_speed_limit)
        {

        case 15:
            strcpy(Led_Yellow.cilentData.data, "Fric:15m/s  p ");
            break;
        case 18:
            strcpy(Led_Yellow.cilentData.data, "Fric:18m/s   ");
            break;
        case 22:
            strcpy(Led_Yellow.cilentData.data, "Fric:22m/s   ");
            break;
        case 30:
            strcpy(Led_Yellow.cilentData.data, "Fric:30m/s   ");
            break;				
        }
				if(Robot_Receive.Write_Msg[5] == 0)// 摩擦轮失能
				{
...
void LEDGreen_MeanUpdate(void)
{
    ...
    switch (Robot_Receive.Write_Msg[6])
    {
    case 0:  //--- 关
        strcpy(Led_Green.cilentData.data, "Magazine:OFF ");
        break;

    case 1:  //--- 开
        strcpy(Led_Green.cilentData.data, "Magazine:ON ");
        break;
...
void LEDOrange_MeanUpdate(void)
{
    ...
    switch (Robot_Receive.chassismode)
    {
    case CHAS_followMode:
        strcpy(Led_Orange.cilentData.data, "ChassisMode:Follow");
        break;
    ...
    case CHAS_NotfollowMode:
			strcpy(Led_Orange.cilentData.data, "ChassisMode:CHAS_NotfollowMode");
        break;		
    default:
        break;
    }
...
void LEDPurple_MeanUpdate(void)
{
    ...
    if(Robot_Receive.Write_Msg[0] == 1)//视觉在线
    {
        switch (Robot_Receive.Vision_Mode)
        {
        case Vision_BIGWindmill:
            strcpy(Led_Purple.cilentData.data, "VisionMode:BIGWindmill ");
            break;

        case Vision_Top:
            strcpy(Led_Purple.cilentData.data, "VisionMode:Top          ");
            break;

        case Vision_Forcast:
            strcpy(Led_Purple.cilentData.data, "VisionMode:Forcast      ");
            break;
        }
    }
		else  //--- 离线
		{
			strcpy(Led_Purple.cilentData.data, "VisionMode:Offline      ");
		}
...
void Capacitance_Value(void)
{
    ...
    int_into_char(Cap_Energyvalue.cilentData.data,Get_Cell());
...
void Cap_EnergyUpdata(void)
{
    ...
    Cell = 5.f / 3.f * (Get_Cell()-40);//映射
    int_into_char(Cap_Energyvalue.cilentData.data, Cell);
