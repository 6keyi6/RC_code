...
TIM_HandleTypeDef* TIMx; //--- TIMx
uint32_t TIM_CHANNEL_x;  //--- TIM通道
 ...
 * @retval     None
 */
void DEV_Buzzer_Init(TIM_HandleTypeDef* timx, uint32_t tim_channel_x)
{
	TIMx = timx;
 ...
 * @retval     None
 */
void Set_RingType(int8_t ringtype)
{
	Ring_Type = ringtype;
}

uint16_t buzzer_val = 300;
void DEV_Buzzer_Process()
{
	...
	{
	case Ring_Stop:	//--- 停止
		HAL_TIM_PWM_Stop(TIMx, TIM_CHANNEL_x);
		state_cnt = 0;
		break;
	...
	case Ring_ContLong: //--- 持续响铃 长间隔
		...
		{
			HAL_TIM_PWM_Start(TIMx, TIM_CHANNEL_x);
			__HAL_TIM_SET_AUTORELOAD(TIMx, buzzer_val);
		}
		else if((state_cnt%=5)==0 && (state_cnt<time_cnt*3))
		{
			__HAL_TIM_SET_AUTORELOAD(TIMx, 0);
		}
		break;
...
{
	HAL_TIM_PWM_Start(TIMx, TIM_CHANNEL_x);
	for (int i = 0; i < len / sizeof(MusicNote_t); i++)
	{
		if (music[i].note == 0)
		{
			__HAL_TIM_SET_AUTORELOAD(TIMx, 0);
		...
		{
			__HAL_TIM_SET_AUTORELOAD(TIMx, 100000 / music[i].note);
			osDelay(music[i].time);
		}
	}
	HAL_TIM_PWM_Stop(TIMx, TIM_CHANNEL_x);
